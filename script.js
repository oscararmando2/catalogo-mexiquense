...
const importModalInner = document.getElementById('importModalInner');
...
function showProductDetails(productId){
    ...
    try{
        ...
        const itemNumberEl = document.getElementById('productItemNumber');
        if(itemNumberEl) itemNumberEl.textContent = sanitizeInput(product.itemNumber || 'N/A');
...
complete: (results) => {
  ...
  results.data.forEach((row, idx) => {
    try {
      ...
      // Clasificar: actualizar, agregar o error
      ...
      // Also check in already-classified toAdd products (to handle duplicates within the CSV)
      if (!existing) {
        existing = toAdd.find(p =>
          (p.itemNumber && p.itemNumber === newProduct.itemNumber) ||
          (p.upc && p.upc === newProduct.upc)
        );
      }
      ...
      if (existing) {
        toUpdate.push({
          ...
          existingDateAdded: existing.dateAdded || newProduct.dateAdded,
        ...
        if (isDuplicateItemCode(newProduct.itemNumber)) {
          errors.push({ row: idx + 1, itemNumber: newProduct.itemNumber, nombre: newProduct.nombre, upc: newProduct.upc, reason: `Código duplicado: "${newProduct.itemNumber}"` });
        } else {
      ...
      console.error(`Error en fila ${idx + 1}:`, err);
      errors.push({ row: idx + 1, itemNumber: '', nombre: '', upc: '', reason: `Error inesperado en fila ${idx + 1}` });
    }
  });

  csvPreviewData = { toAdd, toUpdate, errors };
  renderImportPreview();
  importLoader.classList.add('hidden');
  importSubmitBtn.disabled = false;
},
...
function renderImportPreview() {
  const { toAdd, toUpdate, errors } = csvPreviewData;

  // Resumen de estadísticas
  importPreviewSummary.innerHTML =
    `<span class="inline-flex items-center gap-1 bg-green-100 text-green-800 px-3 py-1 rounded-full">` +
    `<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>` +
    `${toAdd.length} nuevos</span> ` +
    `<span class="inline-flex items-center gap-1 bg-blue-100 text-blue-800 px-3 py-1 rounded-full">` +
    `<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>` +
    `${toUpdate.length} a actualizar</span>` +
    (errors.length > 0
      ? ` <span class="inline-flex items-center gap-1 bg-red-100 text-red-800 px-3 py-1 rounded-full">` +
        `<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M12 2a10 10 0 100 20A10 10 0 0012 2z"/></svg>` +
        `${errors.length} con error</span>`
      : '');
  ...
  toAdd.forEach(p => {
    html += `<tr class="hover:bg-green-50">` +
      `<td class="px-3 py-2 border-b"><span class="inline-block bg-green-100 text-green-800 text-xs font-semibold px-2 py-0.5 rounded">NUEVO</span></td>` +
      `<td class="px-3 py-2 border-b text-gray-700">${p.itemNumber || '—'}</td>` +
      `<td class="px-3 py-2 border-b text-gray-700">${p.nombre || p.description || '—'}</td>` +
      `<td class="px-3 py-2 border-b text-gray-500 text-xs">${p.upc || '—'}</td>` +
      `<td class="px-3 py-2 border-b text-gray-400 text-xs">—</td>` +
      `</tr>`;
  });
  toUpdate.forEach(u => {
    const p = u.product;
    html += `<tr class="hover:bg-blue-50">` +
      `<td class="px-3 py-2 border-b"><span class="inline-block bg-blue-100 text-blue-800 text-xs font-semibold px-2 py-0.5 rounded">ACTUALIZAR</span></td>` +
      `<td class="px-3 py-2 border-b text-gray-700">${p.itemNumber || '—'}</td>` +
      `<td class="px-3 py-2 border-b text-gray-700">${p.nombre || p.description || '—'}</td>` +
      `<td class="px-3 py-2 border-b text-gray-500 text-xs">${p.upc || '—'}</td>` +
      `<td class="px-3 py-2 border-b text-gray-400 text-xs">—</td>` +
  ...
  errors.forEach(err => {
    html += `<tr class="bg-red-50 hover:bg-red-100">` +
      `<td class="px-3 py-2 border-b"><span class="inline-block bg-red-100 text-red-800 text-xs font-semibold px-2 py-0.5 rounded">ERROR</span></td>` +
      `<td class="px-3 py-2 border-b text-gray-700">${err.itemNumber || '—'}</td>` +
      `<td class="px-3 py-2 border-b text-gray-700">${err.nombre || '—'}</td>` +
      `<td class="px-3 py-2 border-b text-gray-500 text-xs">${err.upc || '—'}</td>` +
      `<td class="px-3 py-2 border-b text-red-600 text-xs">${err.reason}</td>` +
      `</tr>`;
  });
  importPreviewBody.innerHTML = html;

  // Mostrar paso 2, ocultar paso 1, ampliar modal
  importStep1.classList.add('hidden');
  importStep2.classList.remove('hidden');
  importModalInner.classList.remove('max-w-md');
  importModalInner.classList.add('max-w-4xl');
}

function confirmImport() {
  if (!csvPreviewData) return;
  const { toAdd, toUpdate } = csvPreviewData;

  // Aplicar actualizaciones a productos existentes (incluyendo duplicados dentro del mismo CSV)
  toUpdate.forEach(u => {
    const { product, existingId, existingDateAdded, existingCustomFields } = u;
    const existing = products.find(p => p.id === existingId) || toAdd.find(p => p.id === existingId);
    if (existing) {
      const mergedCustomFields = { ...existingCustomFields, ...product.customFields };
  ...
  if (toUpdate.length > 0 && toAdd.length > 0) {
    msg = `Importados ${toAdd.length} productos nuevos y ${toUpdate.length} actualizados correctamente.`;
  } else if (toUpdate.length > 0) {
    msg = `Actualizados ${toUpdate.length} productos correctamente.`;
  } else {
    msg = `Importados ${toAdd.length} productos correctamente.`;
  }
  showToast(msg);
...
function resetImportModal() {
  ...
  importModalInner.classList.remove('max-w-4xl');
  importModalInner.classList.add('max-w-md');
  importForm.reset();
...
function setupEventListeners(){
    ...
    document.getElementById('closeImportModal').addEventListener('click', ()=>{ importModal.classList.add('hidden'); csvPreviewData = null; resetImportModal(); });
    ...
    // Import CSV
    importBtn.addEventListener('click', ()=>{ resetImportModal(); csvPreviewData = null; importModal.classList.remove('hidden'); });
    ...
    document.getElementById('importBackBtn').addEventListener('click', ()=>{ csvPreviewData = null; importStep1.classList.remove('hidden'); importStep2.classList.add('hidden'); importModalInner.classList.remove('max-w-4xl'); importModalInner.classList.add('max-w-md'); });
    ...
    window.addEventListener('click', (e)=>{ if(e.target===productDetailModal) productDetailModal.classList.add('hidden'); if(e.target===productFormModal) productFormModal.classList.add('hidden'); if(e.target===importModal){ importModal.classList.add('hidden'); csvPreviewData = null; resetImportModal(); } if(e.target===passwordModal) passwordModal.classList.add('hidden'); });
...
(function enableInlineUrlEdit(){
    ...
    // Use data attribute to track if listener is already bound
